...
export const useBilingualText = (englishText, hindiText = null) => {
  ...
  useEffect(() => {
    const updateDisplayText = async () => {
      ...
      } else if (isBilingual && currentLang === 'en' && hindiText) {
        // When bilingual mode is on and we're in English, show both languages
        const translated = await translateText(englishText, 'en', 'hi');
        setDisplayText(`${englishText} / ${translated}`);
      } else if (isBilingual && currentLang === 'hi' && englishText) {
        // When bilingual mode is on and we're in Hindi, show both languages
        const translated = await translateText(hindiText, 'hi', 'en');
        setDisplayText(`${hindiText} / ${translated}`);
      } else {
        setDisplayText(englishText);
      }
    };
